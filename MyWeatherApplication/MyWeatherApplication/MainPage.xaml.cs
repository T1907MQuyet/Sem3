...
namespace MyWeatherApplication
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        //mot su kien goi ham async thi phai goi mot ham async
        private async void Button_Click(object sender, RoutedEventArgs e)
        {
            RootObject myWeather = await GetWeatherData.getOpenWeather();
            string icon = string.Format("http://openweathermap.org/img/w/{0}.png", myWeather.weather[0].icon);//ket qua tra ve no chi la mot string khong phai anh
